...
export function buildReconstructionManifest(input: ReconstructionPackageInput) {
  ...
  return {
    schemaVersion: "1.2",
    generator: { name: "砚台", version: "0.6.0" },
  ...
  return {
    ...
    preSpecAssessment: {
      objectClass: {
        ...
        notes: "Seeded from Yantai schema 1.2. Confirm every inferred value before implementation."
      },
...
export function createReconstructionPackage(input: ReconstructionPackageInput): Uint8Array {
  ...
  const files: Record<string, Uint8Array> = {
    ...
    "assessment/visual-lens-seed.json": jsonBytes(buildObjectSculptSpecSeed(input)),
    "analysis/visual-lens-analysis.json": jsonBytes({ ...input.result, previewDataUrl: undefined }),
...
function buildReadme(readiness: ReconstructionReadiness): string {
  return `# 砚台 -> img2threejs 交接包

就绪度：${readiness.label}（${Math.round(readiness.score * 100)}%）

1. 先阅读 \`manifest.json\` 的证据边界和缺失视图。
2. \`assessment/visual-lens-seed.json\` 是确定性种子，不是已验证的最终 ObjectSculptSpec。
...
function buildWindowsLauncher(targetName: string, sourcePath: string): string {
  ...
  return [
    ...
    "echo Draft created. Review manifest.json and assessment\\visual-lens-seed.json, then complete and validate the upstream ObjectSculptSpec.",
    "endlocal",
    ""
  ].join("\r\n");
}
