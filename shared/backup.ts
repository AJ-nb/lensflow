...
export function createVisualLensBackup(input: {
  ...
  const { apiKey, ...settingsWithoutKey } = normalized;
  return {
    format: VISUAL_LENS_BACKUP_FORMAT,
    schemaVersion: CURRENT_BACKUP_SCHEMA_VERSION,
    extensionVersion: input.extensionVersion,
    exportedAt: input.exportedAt ?? new Date().toISOString(),
    settings: input.includeApiKey ? { ...settingsWithoutKey, apiKey } : settingsWithoutKey,
...
export function parseVisualLensBackup(value: unknown): VisualLensBackup {
  ...
  if (header.format !== VISUAL_LENS_BACKUP_FORMAT) throw new Error("这不是砚台备份文件。");
  ...
  return parsed.data as unknown as VisualLensBackup;
}
...
export function applyBackupSettings(current: AppSettings, backup?: BackupSettings): AppSettings {
  ...
  const includesApiKey = typeof backup.apiKey === "string";
  return normalizeSettings({
    ...current,
    ...backup,
    apiKey: backup.apiKey ?? current.apiKey,
    rememberApiKey: includesApiKey
      ? backup.rememberApiKey ?? current.rememberApiKey
      : current.rememberApiKey
  });
}
