...
async function consumeCaptureIntent(): Promise<CaptureIntent | null> {
  if (typeof browser === "undefined" || !browser.storage?.session) return null;
  const stored = await browser.storage.session.get(STORAGE_KEYS.captureIntent);
  await browser.storage.session.remove(STORAGE_KEYS.captureIntent);
  const value = stored[STORAGE_KEYS.captureIntent];
  return value === "analyze" || value === "analyze-generate" ? value : null;
}

async function clearCaptureIntent(): Promise<void> {
...
export default function App() {
  ...
  const pendingCaptureIntentRef = useRef<CaptureIntent | null>(null);
  ...
  useEffect(() => {
    ...
    const sidePanelPort = runtime.connect({ name: "lensflow-sidepanel" });
    const listener = (message: { type?: string; source?: ImageSource; intent?: CaptureIntent }) => {
      if (message.type === "SELECTION_UPDATED" && message.source && message.source.id !== activeSourceIdRef.current) {
        ...
        void clearCaptureIntent().then(() => acceptSource(nextSource, true, message.intent ?? null));
      }
    };
  ...
  async function initialize() {
    try {
      ...
      if (session.source) {
        ...
        if (!session.result && captureIntent) {
          await runCaptureIntent(session.source, loadedSettings, captureIntent);
        } else if (!session.result && !session.overview && loadedSettings.autoAnalyze && loadedSettings.apiKey && loadedSettings.apiBaseUrl) {
  ...
  async function acceptSource(nextSource: ImageSource, shouldAutoAnalyze = true, captureIntent: CaptureIntent | null = null) {
    ...
    setError("");
    if (captureIntent) {
      await runCaptureIntent(nextSource, effectiveSettings, captureIntent);
    } else if (shouldAutoAnalyze && effectiveSettings.autoAnalyze && effectiveSettings.apiKey && effectiveSettings.apiBaseUrl) {
      await runPreferredAnalysis(nextSource, effectiveSettings);
    }
  }

  async function runCaptureIntent(target: ImageSource, effectiveSettings: AppSettings, intent: CaptureIntent) {
    setAnalysisChoice("full");
    if (!effectiveSettings.apiBaseUrl || !effectiveSettings.apiKey) {
      setStatus("等待配置 Provider 后反推提示词");
      setView("settings");
      showFeedback("warning", "需要先配置 Provider", intent === "analyze-generate"
        ? "配置完成后返回分析页生成完整提示词，再进入创作空间确认生图。"
        : "配置完成后返回分析页生成完整提示词。");
      return null;
    }
    if (!await hasRemoteAccess(effectiveSettings, target)) {
      setStatus("等待授权后反推提示词");
      showFeedback("warning", "等待访问授权", "点击完整分析的生成按钮后，插件才会申请当前图片和 Provider 的访问权限。");
      return null;
    }
    return analyze(target, "", effectiveSettings);
  }
  ...
  async function runPreferredAnalysis(target: ImageSource, effectiveSettings: AppSettings) {
    if (!await hasRemoteAccess(effectiveSettings, target)) {
      ...
      showFeedback(
        "warning",
        "自动分析已暂停",
        "新域名必须由你主动授权。点击下方生成按钮后，Lensflow 才会申请权限并发送图片。"
      );
      return null;
    }
  ...
  async function analyze(
    ...
    try {
      ...
      try {
        ...
        await completeCaptureIntent(nextResult);
      } catch (archiveError) {
  ...
  async function completeCaptureIntent(nextResult: AnalysisResult) {
    const intent = pendingCaptureIntentRef.current;
    if (!intent) return;
    pendingCaptureIntentRef.current = null;
    if (intent !== "analyze-generate") return;
    const prompt = nextResult.analysis.reconstruction?.positivePrompt?.trim();
    if (!prompt) {
      showFeedback("warning", "未生成可用提示词", "分析结果已保留，请补充提示词后再进入创作空间。");
      return;
    }
    sessionStorage.setItem("lensflow-studio-handoff", JSON.stringify({
      body: prompt,
      activeStep: 2,
      selectedAssetId: `capture:${nextResult.source.id}`
    }));
    if (location.pathname.endsWith("sidepanel.html") && nextResult.source.tabId !== undefined && typeof browser !== "undefined") {
      await browser.sidePanel.setOptions({ tabId: nextResult.source.tabId, path: "sidepanel.html", enabled: true });
    }
    location.hash = "";
    location.reload();
  }
    ...
    try {
      ...
      const generatedReference: ReferenceImage = {
        ...
        source: {
          ...
          fileName: "lensflow-three-view.png"
        },
  ...
  async function exportReconstructionPackage() {
    ...
    try {
      ...
      downloadBlob(new Blob([buffer], { type: "application/zip" }), "lensflow-img2threejs-handoff.zip");
  ...
  async function exportBackup() {
    ...
    try {
      ...
      downloadBlob(
        ...
        `lensflow-backup-${date}.json`
      );
      setStatus("完整备份已导出");
      showFeedback(
        "success",
        "备份已下载",
        `${archive.analyses.length} 个档案 · ${archive.promptVersions.length} 个提示词版本 · 密钥已排除`
      );
    } catch (caught) {
      showError(caught);
    } finally {
    ...
    <main className="app-shell" data-theme={activeTheme.id} onDragOver={(event) => event.preventDefault()} onDrop={(event) => {
      ...
      <header className="app-header">
        <div className="brand-block">
          <img className="brand-mark" src="/icon-128.png" alt="Lensflow Logo" width={36} height={36} />
          <div><h1>镜序 Lensflow</h1><p>v{APP_VERSION}</p></div>
        </div>
...
function JsonView({ result, onCopy, onFeedback }: { result: AnalysisResult; onCopy: (value: string, label: string) => Promise<void>; onFeedback: FeedbackHandler }) {
  ...
  return <div className="json-view"><div className="json-actions"><button className="button secondary" onClick={() => void onCopy(json, "完整 JSON")}><Copy size={16} />复制</button><button className="button secondary" onClick={() => { downloadText(json, "lensflow-analysis.json"); onFeedback("success", "JSON 已下载", "文件名：lensflow-analysis.json"); }}><Download size={16} />下载</button></div><pre>{json}</pre></div>;
}
...
function SettingsView({ settings, showKey, setShowKey, onChange, onSave, onTest, onExportBackup, onImportBackup, testResult, busy }: {
  ...
  return <section className="settings-workspace">
    <div className="settings-heading"><h2>模型与连接</h2><p>由你选择 API 服务商，Lensflow 不提供默认端点</p></div>
...
function DataManagement({ busy, onExport, onImport }: {
  ...
  return <section className="data-management" aria-labelledby="data-management-title">
    ...
    <p className="backup-footnote">API Key 与“记住密钥”设置始终排除，不进入任何备份文件。</p>
    <button className="button secondary backup-action" disabled={disabled} onClick={onExport}>
