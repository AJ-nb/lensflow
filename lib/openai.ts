...
export async function editWithOpenAI(
  ...
  const client = createClient(settings);
  const upload = await toFile(image, "visual-lens-source.png", { type: image.type || "image/png" });
  const response = await runOpenAIRequest(
    settings,
    "图片编辑",
    () => client.images.edit({
...
export async function generateThreeViewWithOpenAI(
  ...
  const client = createClient(settings);
  const upload = await toFile(image, "visual-lens-reference.png", { type: image.type || "image/png" });
